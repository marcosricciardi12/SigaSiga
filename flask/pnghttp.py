...
#     '-i', 'pipe:',  # Input del frame desde Python
#     '-filter_complex', '[1:v]format=rgba,colorchannelmixer=aa=1[overlay];[0:v][overlay]overlay=format=auto,format=yuv420p[v]',
#     '-map', '[v]',
#     '-c:v', 'libx264',
#     '-preset', 'ultrafast',
#     '-tune', 'zerolatency',
...
# ]
    ffmpeg_command = [
    'ffmpeg',
    '-r', '30',  # Aumentar el FPS a 30
    '-i', 'http://192.168.54.139:4747/video',
    '-f', 'image2pipe',  # Indicar que la entrada es un pipe
    '-pix_fmt', 'rgba',  # Formato de píxeles para la transparencia
    '-i', 'pipe:',  # Input del frame desde Python
    '-filter_complex', '[1:v]format=rgba,colorchannelmixer=aa=1[overlay];[0:v][overlay]overlay=format=auto,format=yuv420p[v]',
    '-map', '[v]',
    '-c:v', 'libx264',
    '-preset', 'superfast',  # Cambiar el preset para optimizar la velocidad
    '-tune', 'zerolatency',
    '-f', 'flv',
    'rtmp://localhost/live/stream'
]
